...
function convertDurationToNumberOfMinutes (durationAsString) {
  const minutes = durationAsString.split(" ").reduce((accum, currVal) => {
    
    // console.log(`${parseInt(currVal)} ==> type: ${typeof currVal} ==> ${currVal}`)

    if(currVal.includes("h")){
      return accum + parseInt(currVal) * 60
    } else {
      return accum + parseInt(currVal)
    }
  }, 0);
...
function turnHoursToMinutes(moviesArray) {
  const updatedMoviesArray = moviesArray.map(movie => {
    ...
    // newMovieObject.duration = convertDurationToNumberOfMinutes(movie.duration);
    // newMovieObject.genre = movie.genre;
    // newMovieObject.score = movie.score;

    // return newMovieObject;

    return {
      ...movie, // copy the movie object
      duration:convertDurationToNumberOfMinutes(movie.duration) // change only the value of "duration" property
    }
  })
